...
module LendingPoolV2 {
    ...
    struct PoolInfo<PoolType> has key, store {
    ...
    struct PoolMinBorrow<PoolType> has key, store {
        min_borrow: u128,
    }
    ...
    struct TotalCollateral<PoolType, CollateralTokenType> has key, store {
    ...
    struct TotalBorrow<PoolType, BorrowTokenType> has key, store {
    ...
    // user position
    struct Position<PoolType> has key, store { collateral: u128, borrow: u128 }
    ...
    // cap
    struct SharedRebaseModifyCapability<T> has key, store { cap: Rebase::ModifyCapability<T> }
