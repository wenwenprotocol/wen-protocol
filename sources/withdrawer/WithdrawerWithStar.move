...
module WenProtocol::WithdrawerWithStar {
    ...
    fun do_swap(account: &signer, amount: u128): u128 acquires Reserve, SwapInfo {
        ...
        // wen -> stc
        let amount_y = TokenSwapScripts::get_amount_out<WEN, STC>(amount_x);
        TokenSwapRouter::swap_exact_token_for_token<WEN, STC>(account, amount_x, amount_y);
        // stc -> share
        let amount_z = TokenSwapScripts::get_amount_out<STC, SHARE>(amount_y);
        TokenSwapRouter::swap_exact_token_for_token<STC, SHARE>(account, amount_y, amount_z);
